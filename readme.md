...
仓库保留 C1–C4、校准夹具和报告，以记录真实的研究结果与失败候选：在匹配固定 DCA 的历史样本中，部分候选主要改变现金使用率、回撤或波动，并未稳定形成收益优势。旧模型现已作为版本化内置策略保留，`FixedDcaPolicy` 已成为新计划默认值和公平对照基准。受限 DSL 现已具备确定性、无 IO 的解释器，并由历史评估器直接调用；策略保存、HTTP API 与 Studio 仍未实现。

| 能力 | 当前状态 | 边界 |
| :--- | :--- | :--- |
| 计划管理、周期规则与本地 SQLite | 已完成 | 单用户、本地数据；已有计划保持现有行为。 |
| 70/20 市场输入与 Qwen 证据 | 已完成 | 外部源不可用时明确降级或拒绝本次自动决策，不伪造输入。 |
| 决策存证与历史查询 | 已完成 | 新记录保存策略 ID/版本、通用推荐、输入、结果、Qwen 理由/新闻/警告及可选订单回执；旧记录仍可读。 |
...
| 受限 DSL 定义、校验与确定性运行时 | 已完成研究基础 | 仅允许白名单指标、有限表达式和机会桶动作；首条命中规则在完整快照上无 IO 执行，历史评估复用同一 runtime；尚未接入存储、HTTP 或 Studio。 |
...
6. **策略存储、API 与 Studio**：后续为 DSL 增加版本化保存、验证、回测、激活、Decision Preview 与只读审计页面。
