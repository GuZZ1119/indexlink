...
当前 MVP 会在 Decision Preview 中以计划已保存的 core/opportunity 比例生成建议：核心桶保留，机会桶按有界决策倍率减量或加码，并保存审计快照。`carry_forward` 会在自动来源的 paper order 被接受后把未分配机会预算写入本地 SQLite 独立账本，供后续建议在 `max_single_execution` 硬上限内使用；自动来源的 paper buy 会以建议金额、最新本机 OpenD 收盘价和账户 buying power 换算保守整股数量。scheduler 只生成存证，仍不会自动下单。
...
5. **下一阶段：周期预算与成交对账**：计划已支持月度/周度多个固定执行日和机会资金滚存；后续新增每计划周期预算上限、停机补跑、延迟重审，以及按真实成交金额而非下单估值回写机会现金。
