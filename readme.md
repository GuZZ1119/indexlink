...
当前 MVP 会在 Decision Preview 中以计划已保存的 core/opportunity 比例生成建议：核心桶保留，机会桶按有界决策倍率减量或加码（无滚存余额时最多使用当期机会预算），并保存审计快照。`carry_forward` 当前只保存用户滚存意图；**不会**保存副桶余额、调拨现金、自动补投或自动改变订单金额。以下规则是后续执行层的设计目标，不应视为当前演示行为。
