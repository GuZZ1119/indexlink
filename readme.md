...
- 💰 **双桶建议金额**：每个计划持久化核心/机会比例、审批方式与未使用机会预算意图；到期预览会保留核心桶、以 70/20/10 倍率调整机会桶，并记录建议金额与未分配机会预算。现金池余额、滚存上限与自动补投仍在后续阶段。
...
| 双桶建议金额与 paper order | 部分完成 | 核心桶不会被普通趋势/AI 否决；机会桶按有界倍率建议投入，未使用金额仅记录策略意图，尚无现金池账本或自动补投。仅命中执行日且操作者显式请求时才提交，且只允许模拟账户。 |
| 本地收益账本与图表 | 已完成 | 仅覆盖账本启用后、可归属到计划的已观察成交；不会反推历史账户交易。 |
| 一年历史比较图 | 已完成 | 是基于 MA200 的价格规则回放，**不是**完整 70/20/10 历史回测。 |

尚未实现：计划时区与交易日历、期数型机会现金上限、跨周期停机补跑、自动下单/撤单、完整的历史 70/20/10 回测、云端多用户同步、税费/汇率/分红处理及任何实盘交易能力。
...
当前 MVP 会在 Decision Preview 中以计划已保存的 core/opportunity 比例生成建议：核心桶保留，机会桶按有界决策倍率减量或加码，并保存审计快照。`carry_forward` 会在自动来源的 paper order 被接受后把未分配机会预算写入本地 SQLite 独立账本；`carry_with_cap` 可设金额上限。自动来源的 paper buy 会以建议金额、最新本机 OpenD 收盘价和账户 buying power 换算保守整股数量，并先原子预留周/月累计额度。终态 fill 会按实际成交量与均价回写机会现金和周期占用；pending/partial 保守保留估计。scheduler 会补跑当前周期漏掉的审计日期，但只生成存证，仍不会自动下单。
...
5. **下一阶段：评分、AI 与界面校准**：周期预算、金额型滚存上限、当前周期补跑和终态成交回写已完成；接下来先校验 70/20/10 的分数分布与动作比例，再升级 Qwen 为受限解释层，最后同步 API 与 Web 配置/审计展示。
