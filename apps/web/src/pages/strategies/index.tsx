...
import { CheckCircle2, Copy, Play, Plus, Save, ShieldCheck, Sparkles, Trash2 } from 'lucide-react'
import { useTranslation } from 'react-i18next'

import { useActivatePlanPolicy, useAiProviders, useCopilotDraft, useCreateStrategy, usePlans, useStrategies, useStrategyAdmission, useValidateStrategy } from '@/api/queries'
import type { CopilotDraftResponse, StrategyAdmissionReport, StrategyComparisonDocument, StrategyConditionDocument, StrategyIndicatorDocument, StrategyRuleDocument, StrategySpecDocument } from '@/api/types'
...
/** Generate an unpersisted candidate and return control to the existing explicit Studio workflow. */
function CopilotDraftPanel({ document, setDocument }: { document: StrategySpecDocument; setDocument: Dispatch<SetStateAction<StrategySpecDocument>> }) {
  const { t } = useTranslation()
  const providers = useAiProviders()
  const draft = useCopilotDraft()
  const [profileId, setProfileId] = useState('')
  const [objective, setObjective] = useState('')
  const [result, setResult] = useState<CopilotDraftResponse | null>(null)
  const capableProfiles = useMemo(() => providers.data?.providers.filter((profile) => profile.capabilities.restricted_policy_drafts) ?? [], [providers.data])
  const selectedProfileId = profileId || capableProfiles[0]?.id || ''

  const generate = async () => {
    if (!selectedProfileId) return
    const next = await draft.mutateAsync({ profile_id: selectedProfileId, policy_id: document.policy_id, policy_version: document.policy_version, objective })
    setResult(next)
  }

  const changes = result ? compareCopilotDraft(document, result.document) : []
  const applyDraft = () => {
    if (!result || !globalThis.confirm(t('studio.copilotApplyConfirm'))) return
    setDocument(result.document)
    setResult(null)
  }

  return <Card className="border-violet-200 bg-violet-50/40"><CardHeader><CardTitle className="flex gap-2"><Sparkles className="size-5 text-violet-700" />{t('studio.copilotTitle')}</CardTitle><CardDescription>{t('studio.copilotDescription')}</CardDescription></CardHeader><CardContent className="space-y-3"><label className="grid gap-1 text-sm font-medium">{t('studio.copilotProvider')}<select className="h-9 rounded-md border bg-background px-2" value={selectedProfileId} onChange={(event) => setProfileId(event.target.value)} disabled={providers.isLoading || capableProfiles.length === 0}><option value="">{providers.isLoading ? t('common.loading') : t('studio.copilotNoProvider')}</option>{capableProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.display_name} · {profile.model}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">{t('studio.copilotObjective')}<textarea className="min-h-24 rounded-md border bg-background p-2 text-sm" value={objective} maxLength={600} onChange={(event) => setObjective(event.target.value)} placeholder={t('studio.copilotObjectivePlaceholder')} /></label>{providers.error && <p className="text-sm text-destructive">{t('studio.copilotProviderFailed')}</p>}{draft.error && <p className="text-sm text-destructive">{draft.error instanceof Error ? draft.error.message : t('studio.copilotFailed')}</p>}<Button className="w-full" type="button" disabled={!selectedProfileId || !objective.trim() || draft.isPending} onClick={() => void generate()}><Sparkles className="mr-2 size-4" />{draft.isPending ? t('studio.copilotGenerating') : t('studio.copilotGenerate')}</Button>{result && <div className="space-y-3 rounded-lg border border-violet-200 bg-background p-3 text-sm"><p className="font-medium">{t('studio.copilotGeneratedBy', { provider: `${result.provider.display_name} · ${result.provider.model}` })}</p><p>{result.explanation}</p><section><p className="font-medium">{t('studio.copilotReview')}</p>{changes.length === 0 ? <p className="mt-1 text-xs text-muted-foreground">{t('studio.copilotNoChanges')}</p> : <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-muted-foreground">{changes.map((change) => <li key={change.field}>{t('studio.copilotChange', { field: change.field, kind: t(`studio.copilotChange.${change.kind}`) })}</li>)}</ul>}</section>{result.warnings.length > 0 && <section><p className="font-medium">{t('studio.copilotWarnings')}</p><ul className="mt-1 list-disc space-y-1 pl-5 text-muted-foreground">{result.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></section>}<section><p className="font-medium">{t('studio.copilotEvidence')}</p><ul className="mt-1 space-y-1 text-xs text-muted-foreground">{result.evidence.map((reference) => <li key={reference.id}>{reference.label}</li>)}</ul></section><Button type="button" className="w-full" variant="outline" onClick={applyDraft}>{t('studio.copilotApply')}</Button><p className="rounded-md bg-muted p-2 text-xs text-muted-foreground">{t('studio.copilotNext')}</p></div>}</CardContent></Card>
}
