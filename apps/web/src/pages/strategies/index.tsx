...
export default function StrategiesPage() {
  ...
  const runAdmission = () => {
    if (!selectedStrategy) return
    setAdmissionPolicy(selectedKey)
    admission.mutate(selectedStrategy.policy)
  }

  return <div className="mx-auto grid w-full max-w-7xl gap-4 p-4 lg:grid-cols-[18rem_minmax(0,1fr)_22rem] lg:p-6">
    <Card><CardHeader><CardTitle>策略版本</CardTitle><CardDescription>不可变版本；复制后创建新版本。</CardDescription></CardHeader><CardContent className="space-y-2">{strategies.data?.map((item) => <button key={`${item.policy.id}@${item.policy.version}`} type="button" onClick={() => { setSelected(`${item.policy.id}@${item.policy.version}`); setAdmissionPolicy(null) }} className="w-full rounded-lg border p-3 text-left hover:bg-muted/50"><strong>{item.name}</strong><span className="mt-1 block font-mono text-xs text-muted-foreground">{item.policy.id}@{item.policy.version}</span></button>)}<Button className="w-full" variant="outline" onClick={duplicate} disabled={!selectedStrategy}><Copy className="mr-2 size-4" />复制选中版本</Button></CardContent></Card>
    ...
    <div className="space-y-4"><Card className="border-amber-200 bg-amber-50/40"><CardHeader><CardTitle>激活准入回测</CardTitle><CardDescription>激活前必须通过核心桶/预算检查，并以固定样本在同一投入、成本与成交时点下对照 Fixed DCA。</CardDescription></CardHeader><CardContent className="space-y-3"><Button className="w-full" variant="outline" disabled={!selectedStrategy || admission.isPending} onClick={runAdmission}><Play className="mr-2 size-4" />运行固定样本准入回测</Button>{admission.error && <p className="text-sm text-destructive">{admission.error instanceof Error ? admission.error.message : '准入回测失败'}</p>}{activeAdmission && <AdmissionSummary report={activeAdmission} />}</CardContent></Card><Card><CardHeader><CardTitle>激活到计划</CardTitle><CardDescription>通过准入后，Preview、scheduler 与审计才会运行该版本；审批模式不自动下单。</CardDescription></CardHeader><CardContent className="space-y-3">{selectedStrategy && plans.map((plan) => <div key={plan.id} className="rounded-lg border p-3"><p className="font-medium">{plan.name} · {plan.symbol}</p><p className="text-xs text-muted-foreground">当前 {plan.policy.id}@{plan.policy.version}</p><Button className="mt-3 w-full" size="sm" disabled={!activeAdmission?.eligible || activate.isPending} onClick={() => { if (globalThis.confirm(`绑定 ${selectedStrategy.name} 到 ${plan.name}？`)) activate.mutate({ planId: plan.id, policy: selectedStrategy.policy }) }}>确认激活</Button>{!activeAdmission?.eligible && <p className="mt-2 text-xs text-muted-foreground">请先运行并通过固定样本准入回测。</p>}</div>)}</CardContent></Card></div>
  </div>
}

function AdmissionSummary({ report }: { report: import('@/api/types').StrategyAdmissionReport }) {
  if (!report.eligible) return <p className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">未通过准入：{report.reason ?? '核心桶、预算或历史样本检查未通过。'}</p>
  return <div className="space-y-3 rounded-lg border bg-background p-3 text-sm"><p className="text-emerald-700">通过：核心桶不可否决，固定预算有效。以下是历史固定样本的如实对照，不构成收益承诺。</p>{report.assets.map((asset) => <div key={asset.symbol} className="rounded border p-2"><p className="font-medium">{asset.symbol} · {asset.observations} 次同期观察</p><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><Metric label="策略期末净值" value={`$${asset.strategy.terminal_wealth_usd.toFixed(0)}`} /><Metric label="Fixed DCA 期末净值" value={`$${asset.fixed_dca.terminal_wealth_usd.toFixed(0)}`} /><Metric label="策略最大回撤" value={`${asset.strategy.maximum_drawdown_percent.toFixed(2)}%`} /><Metric label="DCA 最大回撤" value={`${asset.fixed_dca.maximum_drawdown_percent.toFixed(2)}%`} /><Metric label="策略年化波动" value={percent(asset.strategy.annualized_volatility_percent)} /><Metric label="DCA 年化波动" value={percent(asset.fixed_dca.annualized_volatility_percent)} /><Metric label="策略现金使用率" value={`${asset.strategy.cash_utilisation_percent.toFixed(1)}%`} /><Metric label="DCA 现金使用率" value={`${asset.fixed_dca.cash_utilisation_percent.toFixed(1)}%`} /></div></div>)}</div>
}
...
function percent(value: number | undefined) { return value === undefined ? '样本不足' : `${value.toFixed(2)}%` }
