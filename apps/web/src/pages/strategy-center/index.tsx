...
export default function StrategyCenterPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8 px-5 py-8 md:px-8 lg:px-10 lg:py-10">
      <PageHeading eyebrow="策略中心" title="先看懂，再开始坚持" description="每一份策略都明确告诉你它想解决什么、历史上经历过什么，以及最不适合它的情况。" action={<button type="button" className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-[#102028] hover:border-slate-400"><Plus className="size-4" />创建我的策略</button>} />
      ...
      <section><div className="mb-5 flex items-end justify-between gap-4"><div><h2 className="text-xl font-semibold tracking-[-0.03em] text-[#102028]">从简单的方式开始</h2><p className="mt-1 text-sm text-slate-500">目前是本地精选策略库；公开分享与真实回测将在后续版本接入。</p></div><button type="button" className="hidden items-center gap-1 text-sm text-slate-600 sm:inline-flex">风险偏好 <ChevronDown className="size-4" /></button></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{consumerStrategies.map((strategy) => <StrategyCard key={strategy.id} strategy={strategy} selected={activeStrategyId === strategy.id} onSelect={setActiveStrategyId} />)}</div></section>
