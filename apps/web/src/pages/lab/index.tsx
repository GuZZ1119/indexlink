...
export default function LabPage() {
  ...
  const opened = integrations.find((integration) => integration.id === openedIntegration)
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8 px-5 py-8 md:px-8 lg:px-10 lg:py-10">
      ...
      <section className="grid gap-4 md:grid-cols-2">{integrations.map((integration) => <IntegrationCard key={integration.id} integration={integration} active={openedIntegration === integration.id} onOpen={() => setOpenedIntegration(integration.id)} />)}</section>
      ...
      {opened && <section aria-live="polite" className="rounded-[1.35rem] border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-[#2d6a57]">配置预览</p><h2 className="mt-1 text-xl font-semibold tracking-[-0.03em] text-[#102028]">{opened.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{opened.description}</p></div><button type="button" onClick={() => setOpenedIntegration(null)} className="text-sm text-slate-500 hover:text-[#102028]">关闭</button></div><div className="mt-5 rounded-xl bg-[#f6f8f6] p-4 font-mono text-xs leading-6 text-slate-600"><p># 此壳子不写入任何配置</p><p>INDEXLINK_{opened.id.toUpperCase()}_ENABLED=false</p><p>请通过 Docker 环境变量或本地 .env 在服务端完成配置。</p></div></section>}
