...
export function StrategyCard({ strategy, selected, onSelect, compact = false }: { strategy: ConsumerStrategy; selected: boolean; onSelect: (id: ConsumerStrategy['id']) => void; compact?: boolean }) {
  return (
    <article className={cn('relative flex h-full flex-col rounded-[1.35rem] border bg-white p-5 transition-colors', selected ? 'border-[#2d6a57] bg-[#f5faf7]' : 'border-slate-200 hover:border-slate-300')}>
      ...
      {!compact && <div className="mt-5 rounded-xl bg-[#f6f8f6] p-3.5 text-sm leading-6 text-slate-700"><span className="font-medium text-[#102028]">它会怎么做：</span>{strategy.rule}</div>}
      ...
      <button type="button" onClick={() => onSelect(strategy.id)} className="mt-5 inline-flex items-center gap-1 self-start text-sm font-medium text-[#2d6a57] outline-none hover:text-[#1f5444] focus-visible:rounded focus-visible:ring-2 focus-visible:ring-[#2d6a57] focus-visible:ring-offset-2">
        {selected ? '查看我的计划' : '了解并选用'} <ArrowUpRight className="size-3.5" />
      </button>
    </article>
  )
}
