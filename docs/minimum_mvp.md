...
- 第三阶段由 server 配置把本机 OpenD session 注入 Decision Preview，在 demo 中展示“decision preview -> broker paper order -> order ack”。
...
- Axum API server。
- SQLite 本地 storage adapter（PostgreSQL adapter 保留为兼容实现）。
- 健康检查与 readiness 检查。
...
- 已实现本机 loopback OpenD 的 raw TCP session、心跳、模拟账户选择、market/limit 下单与回执校验。
- server 设置 `OPEND_PROVIDER=futu|moomoo` 时，会在启动时初始化真实 paper broker；没有该配置时安全回退到 MockBroker。OpenD 初始化失败会阻止启动，不会静默降级。
- 已支持 mock broker，用于虚拟账号 demo 前的本地闭环测试。
- mock broker 默认拒绝 live orders，必须显式开启才接受 live-mode 请求。

### Decision Preview API + Paper Broker 串联

- 已新增 `POST /investment-plans/:id/decision-preview`。
- API 可接收执行日、双桶比例、fundamental signal、trend signal 和可选 paper order 请求；sentiment 仅由服务端自动获取，不能由调用方绕过。
...
- due 且 action 可执行时，可以通过 broker port 向默认 `MockBroker` 或已配置 OpenD paper broker 提交订单。
- waiting、inactive、`Skip` 和 `TacticalDelay` 不会触发 paper order。
- 响应包含 execution preview、decision result、可选 broker ack 和按执行/三层信号/权重/双桶/订单状态组织的稳定 summary。

### 70% 基本面量化

- `quant-engine` 已有 fundamental 方向的实现和测试。
- 已覆盖 CAPE、ERP、历史分位、权重、错误边界和审计字段。
- 已提供 `POST /signals/fundamental/preview`，以调用方提供的至少 60 个有效月度 CAPE/ERP 快照计算并返回可审计的 70% 主信号。
...
- 已提供 `POST /signals/trend/preview`，以调用方提供的至少 60 个有效月度 MA200 distance、RSI、VIX 快照计算并返回 20% 信号和 regime。
...
- server 可选配置 Qwen，并提供 `POST /market-sentiment/preview`；未配置 Key 时安全返回 `503`。
...
当前 decision preview API 已经能把手工传入的 fundamental、trend、双桶配置和已配置的 paper broker 串成后端闭环，并在决策前自动调用已注入的 Qwen market-sentiment pipeline。成功时使用 `70/20/10`；未配置 Key 或 pipeline 不可用时安全降级为 `90/10/0`，通过 `weight_mode: sentiment_unavailable` 对外说明。请求不再接受手工 sentiment，避免绕过自动链路。

MVP 仍需要人工完成：

- 从受控前端输入或已确认的数据供应商取得 CAPE、ERP、MA200 distance、RSI、VIX 的月度快照；服务端已提供计算、验证和审计字段，但不在未选择数据源时擅自抓取数据。
- 在本机已登录 OpenD 与明确模拟账户配置下，完成一次人工确认的真实 paper-order smoke。

### Futu/Moomoo OpenD 实际虚拟账户 smoke 待执行

当前 broker crate 已实现 Futu/Moomoo OpenD raw TCP transport 与 paper order adapter，server 也可按环境变量完成注入。尚未在开发机连到真实虚拟账户执行一笔人工确认的订单。

MVP 需要补到：

- 准备本机已认证的 Futu/Moomoo OpenD 与一个明确的模拟账户。
- 使用 `OPEND_PROVIDER`、`OPEND_HOST`、`OPEND_PORT`、`OPEND_ACCOUNT_ID` 启动 server；只接受 loopback OpenD。
- 设置显式确认变量、唯一 idempotency key、symbol 与 quantity，运行忽略式 smoke。
- 记录只包含验证结果的 smoke 结论，不记录 account id、订单 id、密码或 token。
...
1. 启动本地 SQLite 文件与 API server（服务启动时自动执行 migration）。
...
6. 调用 `POST /signals/fundamental/preview`，得到 70% 基本面分数和审计分位。
7. 调用 `POST /signals/trend/preview`，得到 20% 趋势节奏分数、regime 和审计分位。
...
14. 返回最终 summary；Decision Preview 已自动写入本地 decision record。
...
## 已具备的 Decision Preview + Paper Broker 子流程

Decision Preview API 覆盖第 8-14 步的本地后端闭环：合成 70/20/10、复用执行预览、输出双桶拆分、按安全条件提交默认 mock 或已配置 OpenD paper order，并返回 summary。
...
2. 真实 provider smoke（需要人工账号/凭据）
   - 使用本地 DashScope Key 执行一次 Qwen network smoke。
   - 使用本机已登录的 Futu/Moomoo OpenD 与模拟账户执行一次人工确认的 paper-order smoke。
   - 选择并授权一个行情/估值数据源，再由前端或受控导入流程提供信号 API 所需的月度快照。

3. 可选：受保护 live trading
...
当前状态更准确地说是：除前端和需要人工提供的外部条件外，MVP 后端已完成。投资计划、SQLite migration、双桶执行预览、fundamental/trend HTTP 信号计算、70/20/10 decision engine、Qwen 自动接入与安全降级、decision record、本机 OpenD paper adapter 和分层 summary 均可运行。剩余是 DashScope Key 真实 smoke、OpenD 虚拟账户真实 smoke，以及选定并授权市场数据来源后的月度快照输入；演示级前端由 Jame 负责。
