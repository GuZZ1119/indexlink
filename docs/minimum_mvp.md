...
演示不能只返回几个分数；需要一份人能看懂的最终 summary。当前 decision preview API 已返回演示级短 summary，并自动写入本地 SQLite decision record，但还需要升级为更完整的分层解释。
...
当前状态更准确地说是：投资计划与双桶执行层可演示；70% fundamental 可复用；20% trend 已可复用；70/20/10 decision engine 已可复用；broker port、mock broker、OpenD paper adapter 与 server 注入的 Decision Preview 已可复用；Decision Preview 已自动调用 Qwen market-sentiment pipeline 并把审计快照写入本机 SQLite history；真实 DashScope Key 网络 smoke、structured explanation、真实虚拟账户 OpenD smoke、完整最终 summary 和演示级前端仍是 MVP 缺口。
